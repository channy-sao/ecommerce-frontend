...
export function ImageUpload({
    ...
    return (
        <div className="space-y-3">
                ...
                <div className="flex-shrink-0">
                    {showNewFile ? (
                        <div className="relative">
                            ...
                            <div
                                className="absolute bottom-1 left-1 bg-blue-600 text-white text-xs px-1 rounded text-[10px]">New
                            </div>
                        </div>
                    ) : showExistingImage ? (
                        <div className="relative">
                            ...
                            <div
                                className="absolute bottom-1 left-1 bg-gray-600 text-white text-xs px-1 rounded text-[10px]">Current
                            </div>
                        </div>
                    ) : (
                        <div
                            className={`w-16 h-16 rounded border-2 border-dashed flex items-center justify-center ${
                                hasRemovedFile ? "border-red-300 bg-red-50" : "border-gray-300 bg-gray-50"
                            }`}
                        >
                            {hasRemovedFile ? <X className="h-6 w-6 text-red-400"/> :
                    ...
                    <div className="space-y-2">
                        <div className="flex items-center gap-2">
                            <Button type="button" variant="outline" size="sm" disabled={disabled} className="h-8">
                                ...
                                {hasRemovedFile ? "Add image" : showExistingImage ? "Replace Image" : "Choose Image"}
                            </Button>

                            {hasNewFile && (
                                <span className="text-sm text-gray-600 truncate flex-1">{(value as File).name}</span>
                            )}
                            {hasRemovedFile &&
                                <span className="text-sm text-blue-600 truncate flex-1">Image removed</span>}
                            {showExistingImage &&
                                <span className="text-sm text-gray-500 truncate flex-1">Current image</span>}
                        </div>
            ...
            {isDragActive && (
                <div
                    className="fixed inset-0 bg-blue-50 bg-opacity-90 border-2 border-dashed border-blue-400 flex items-center justify-center z-50">
                    <div className="text-center p-8 bg-white rounded-lg shadow-lg border">
                        <Upload className="h-10 w-10 text-blue-500 mx-auto mb-3"/>
                        <p className="font-medium text-blue-900">{showExistingImage ? "Drop image to replace" : "Drop image to upload"}</p>
                    </div>
                </div>
            )}
        </div>
